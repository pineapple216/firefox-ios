...
public class BrowserTable: Table {
    ...
    func updateTable(db: SQLiteDBConnection, from: Int) -> Bool {
        ...
        if from < 12 && to >= 12 {
            ...
            let migrateLocal =
            "INSERT INTO \(TableBookmarksLocal) (id, guid, type, bmkUri, title, faviconID, local_modified, sync_status, parentid, parentName) " +
            "SELECT id, guid, type, url, title, faviconID, \(modified) AS local_modified, \(status) AS sync_status" +
